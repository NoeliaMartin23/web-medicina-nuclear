    ...
    <div className={`min-h-screen ${isDarkMode ? 'dark' : ''}`}>
      ...
      <div className="fixed inset-0 z-0">
        <ImageWithFallback
          ...
          className="w-full h-full object-covoveredMenuItem(itemId);
  }, []);
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'dark' : ''}`}>
      ...
      {/* Fixed Header */}
              ...
              {/* Title - Centers ${
                            selectedLanguage === ext-white uppercase">
     -semibold' : ''
                          }`}
                        >
                          {language}
        ...
        {menuItems.map((item) => (
          item.subItems && item.subItems.length > 0 && hoveredMenuItem === item.id && (
            <div
              key={`popup-${item.id}`}
              className="fixedlanguage ? 'bg-gray-100 font-semibold' : ''
                          }`}
                        >
                          {language}
                        </button>
         ...
         {item.subItems && item.subItems.length > 0 && (
                    ...
                    {item.subItems && expandedMenuItems.includes(item.id) && (
                      <div className="ml-4 mt-2 space-y-1 border-l-2 border-[#F5B494]/30 pl-2">
                        {2.5 text-[#F5B494] hover:bg-[#F5B494]/15 hover:text-white text-sm transition-all duration-150 flex items-center gap-2"
                  >
